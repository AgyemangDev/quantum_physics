  ...
  return (
    <div style={{ background: "rgba(255,255,255,0.015)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 12, padding: "14px 16px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
        <span style={{ fontSize: 9, color: "white", textTransform: "uppercase", letterSpacing: "0.12em", fontFamily: "monospace" }}>{title}</span>
        {eq && <div style={{ fontSize: "0.72em", opacity: 1 }}>{eq}</div>}
      </div>
      {children}
      {legend && (
