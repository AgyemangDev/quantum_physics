...
const EMPTY = (
  <div style={{
    ...
    color: "rgba(51,65,85,0.7)", fontFamily: "monospace", fontSize: 12, letterSpacing: "0.06em",
  }}>
...
export default function TimeEvolutionPage() {
  ...
  const showTR = POTENTIAL_HAS_TR[potential] && isLastFrame && !!data;
            ...
            <div style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <button onClick={togglePlay} style={{
                  padding: "6px 20px", fontFamily: "monospace", fontSize: 12, fontWeight: 700,
                ...
                <div style={{ display: "flex", gap: 4, marginLeft: "auto" }}>
                  {SPEEDS.map(s => (
                    <button key={s} onClick={() => setSpeed(s)} style={{
                      padding: "5px 10px", fontFamily: "monospace", fontSize: 11, fontWeight: 700,
                      cursor: "pointer", borderRadius: 5,
                      border: `1px solid ${speed === s ? "#f59e0b" : "rgba(255,255,255,0.07)"}`,
                      background: speed === s ? "rgba(245,158,11,0.09)" : "transparent",
                      color: speed === s ? "#f59e0b" : "rgba(100,116,139,0.7)",
                    }}>
                      {s}×
                    </button>
                  ))}
                ...
                <>
                  ...
                  <div style={{
                    ...
                    <div style={{ fontSize: 10, color: "rgba(100,116,139,0.7)", marginTop: 2 }}>
                      {Math.abs(T + R - 1) < 0.05 ? "≈ 1.0  ✓  conserved" : "≠ 1.0  ✗  check tMax"}
                    </div>
                    <div style={{ marginTop: 8 }}>
                      <Eq display tex={String.raw`T + R = 1`} />
                    </div>
                  </div>
                </>
              )}
